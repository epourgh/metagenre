...
export default function Medium() {
  ...
  const getMediums = () => {
    fetch(`http://18.225.9.206/api/mediums/genreless?mediumType=${paramsMediumType}`)
  ...
  const getMediumsGenres = () => {
    const container = {};
    fetch(`http://18.225.9.206/api/mediumsGenresView?mediumType=${paramsMediumType}`)
        .then(response => response.json())
        .then(response => {
    ...
    fetch(`http://18.225.9.206/api/mediumsSubgenresView?mediumType=${paramsMediumType}`)
        .then(response => response.json())
        .then(response => {
            if (response.data.length > 0) {
    ...
    //     fetch(`http://18.225.9.206/api/mediums/add?title=${medium.title}&mediumType=${medium.mediumType}`)
    //       .then(update => {
    //           getMediums();
    //           getMediumsGenres();
    //       })
    ...
    const voteMediumGenre = (mediumId, mediumGenresId, routeString, mediumGenreVotes) => {
        ...
        fetch(`http://18.225.9.206/api/${routeString}/update/${mediumGenresId}?mediumId=${mediumId}&votes=${votedMediumGenre}`)
            .then(response => {
              getMediums();
              getMediumsGenres();
            })
    ...
    const voteMediumSubgenre = (mediumId, mediumGenresId, routeString, mediumGenreVotes, subgenreId) => {
      ...
      fetch(`http://18.225.9.206/api/${routeString}/update/${mediumGenresId}?mediumId=${mediumId}&votes=${votedMediumGenre}&subgenreId=${subgenreId}`)
        .then(response => {
          getMediums();
          getMediumsGenres();
        })
