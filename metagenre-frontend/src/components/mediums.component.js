...
export default function Medium() {
  ...
  const getMediums = () => {
    fetch(`http://localhost:4000/mediums/genreless?mediumType=${paramsMediumType}`)
  ...
  const getMediumsGenres = () => {
    const container = {};
    fetch(`http://localhost:4000/mediumsGenresView?mediumType=${paramsMediumType}`)
        .then(response => response.json())
        .then(response => {
    ...
    fetch(`http://localhost:4000/mediumsSubgenresView?mediumType=${paramsMediumType}`)
        .then(response => response.json())
        .then(response => {
            if (response.data.length > 0) {
    ...
    //     fetch(`http://localhost:4000/mediums/add?title=${medium.title}&mediumType=${medium.mediumType}`)
    //       .then(update => {
    //           getMediums();
    //           getMediumsGenres();
    //       })
